/* =============================================================
   Kora Visual Editor — editor.js  (v4: assets panel integrated)
============================================================= */
'use strict';
...
const assetsPanel = new AssetsPanel({
  ...
  canvas,
  screenToWorld,
  spawnEntity: (name, type, wx, wy, assetId) => {
    const e = createEntity(name, type);
    e.x = Math.round(wx); e.y = Math.round(wy);
    e.assetId = assetId;
    // If image asset, try to get natural size
    const asset = assetsPanel.get(assetId);
    if (asset?.kind === 'image') {
      const img = new Image();
      img.onload = () => {
        e.w = Math.min(img.naturalWidth,  192);
        e.h = Math.min(img.naturalHeight, 192);
        render();
      };
      img.src = asset.url;
    }
    state.entities.push(e);
    selectEntity(e.id); buildHierarchy(); markDirty();
  },
  onLog: log,
});
...
function drawGrid() {
  const S=32*state.cam.zoom;
  const ox=(canvas.width/2+state.cam.x*state.cam.zoom)%S;
  const oy=(canvas.height/2+state.cam.y*state.cam.zoom)%S;
  ctx.save(); ctx.strokeStyle='rgba(255,255,255,0.04)'; ctx.lineWidth=1;
  for(let x=ox;x<canvas.width;x+=S){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,canvas.height);ctx.stroke();}
  for(let y=oy;y<canvas.height;y+=S){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(canvas.width,y);ctx.stroke();}
  ctx.restore();
}
function drawEntity(e) {
  ...
  const asset = e.assetId ? assetsPanel.get(e.assetId) : null;
  if (asset?.kind==='image') {
    const img = assetImageCache.get(asset.id) || cacheImage(asset);
...
function cacheImage(asset) {
  ...
  img.src = asset.url;
  img.onload = () => render();
  assetImageCache.set(asset.id, img);
  return img;
}
