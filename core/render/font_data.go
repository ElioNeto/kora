package render

import (
	"image"
	"image/color"
	"sync"

	"github.com/hajimehoshi/ebiten/v2"
)

// DefaultFont is the built-in 8×8 monospace bitmap font used by DebugTextAt.
// It is initialised lazily on first use.
var DefaultFont *BitmapFont

var (
	defaultFontOnce sync.Once
)

// initDefaultFont creates the built-in bitmap font atlas from embedded glyph data.
// Called once via sync.Once on first use.
func initDefaultFont() {
	defaultFontOnce.Do(func() {
		const (
			charW  = 8
			charH  = 8
			cols   = 95 // ASCII 32..126
			firstR = rune(32)
		)
		atlas := generateFontAtlas()
		DefaultFont = NewBitmapFont(atlas, firstR, cols, 1, charW, charH)
	})
}

// generateFontAtlas builds an *ebiten.Image from the embedded bitmap font data.
func generateFontAtlas() *ebiten.Image {
	const (
		charW = 8
		charH = 8
		chars = 95 // ASCII 32-126
	)
	width := chars * charW
	height := charH

	// Draw pixels into a standard Go image first.
	img := image.NewRGBA(image.Rect(0, 0, width, height))
	for i, glyph := range bitmapFontData {
		baseX := i * charW
		for row := 0; row < charH; row++ {
			b := glyph[row]
			for col := 0; col < charW; col++ {
				if b&(1<<(7-col)) != 0 {
					img.Set(baseX+col, row, color.White)
				}
			}
		}
	}
	return ebiten.NewImageFromImage(img)
}

// ---------------------------------------------------------------------------
// Embedded 8×8 bitmap font (ASCII 32-126)
// ---------------------------------------------------------------------------
// Each glyph is 8 bytes, one per row. MSB = leftmost pixel.
//
//lint:ignore U1000 font data accessed by generateFontAtlas

// bitmapFontData holds pixel data for ASCII characters 32 through 126.
var bitmapFontData = [95][8]byte{
	// 32  space
	{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
	// 33  !
	{0x18, 0x18, 0x18, 0x18, 0x18, 0x00, 0x18, 0x00},
	// 34  "
	{0x6C, 0x6C, 0x6C, 0x00, 0x00, 0x00, 0x00, 0x00},
	// 35  #
	{0x6C, 0x6C, 0xFE, 0x6C, 0xFE, 0x6C, 0x6C, 0x00},
	// 36  $
	{0x18, 0x3E, 0x60, 0x3C, 0x06, 0x7C, 0x18, 0x00},
	// 37  %
	{0x00, 0x66, 0xAC, 0xD8, 0x36, 0x6A, 0xCC, 0x00},
	// 38  &
	{0x38, 0x6C, 0x68, 0x76, 0xDC, 0xCC, 0x76, 0x00},
	// 39  '
	{0x18, 0x18, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00},
	// 40  (
	{0x0C, 0x18, 0x30, 0x30, 0x30, 0x18, 0x0C, 0x00},
	// 41  )
	{0x30, 0x18, 0x0C, 0x0C, 0x0C, 0x18, 0x30, 0x00},
	// 42  *
	{0x00, 0x66, 0x3C, 0xFF, 0x3C, 0x66, 0x00, 0x00},
	// 43  +
	{0x00, 0x18, 0x18, 0x7E, 0x18, 0x18, 0x00, 0x00},
	// 44  ,
	{0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x30},
	// 45  -
	{0x00, 0x00, 0x00, 0x7E, 0x00, 0x00, 0x00, 0x00},
	// 46  .
	{0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x00},
	// 47  /
	{0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x00},
	// 48  0
	{0x3C, 0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00},
	// 49  1
	{0x18, 0x38, 0x18, 0x18, 0x18, 0x18, 0x7E, 0x00},
	// 50  2
	{0x3C, 0x66, 0x06, 0x0C, 0x30, 0x60, 0x7E, 0x00},
	// 51  3
	{0x3C, 0x66, 0x06, 0x1C, 0x06, 0x66, 0x3C, 0x00},
	// 52  4
	{0x0C, 0x1C, 0x3C, 0x6C, 0x7E, 0x0C, 0x0C, 0x00},
	// 53  5
	{0x7E, 0x60, 0x7C, 0x06, 0x06, 0x66, 0x3C, 0x00},
	// 54  6
	{0x3C, 0x66, 0x60, 0x7C, 0x66, 0x66, 0x3C, 0x00},
	// 55  7
	{0x7E, 0x06, 0x0C, 0x18, 0x30, 0x30, 0x30, 0x00},
	// 56  8
	{0x3C, 0x66, 0x66, 0x3C, 0x66, 0x66, 0x3C, 0x00},
	// 57  9
	{0x3C, 0x66, 0x66, 0x3E, 0x06, 0x66, 0x3C, 0x00},
	// 58  :
	{0x00, 0x18, 0x18, 0x00, 0x00, 0x18, 0x18, 0x00},
	// 59  ;
	{0x00, 0x18, 0x18, 0x00, 0x00, 0x18, 0x18, 0x30},
	// 60  <
	{0x06, 0x0C, 0x18, 0x30, 0x18, 0x0C, 0x06, 0x00},
	// 61  =
	{0x00, 0x00, 0x7E, 0x00, 0x00, 0x7E, 0x00, 0x00},
	// 62  >
	{0x60, 0x30, 0x18, 0x0C, 0x18, 0x30, 0x60, 0x00},
	// 63  ?
	{0x3C, 0x66, 0x06, 0x0C, 0x18, 0x00, 0x18, 0x00},
	// 64  @
	{0x3C, 0x66, 0x6E, 0x6E, 0x60, 0x66, 0x3C, 0x00},
	// 65  A
	{0x18, 0x3C, 0x66, 0x66, 0x7E, 0x66, 0x66, 0x00},
	// 66  B
	{0x7C, 0x66, 0x66, 0x7C, 0x66, 0x66, 0x7C, 0x00},
	// 67  C
	{0x3C, 0x66, 0x60, 0x60, 0x60, 0x66, 0x3C, 0x00},
	// 68  D
	{0x78, 0x6C, 0x66, 0x66, 0x66, 0x6C, 0x78, 0x00},
	// 69  E
	{0x7E, 0x60, 0x60, 0x7C, 0x60, 0x60, 0x7E, 0x00},
	// 70  F
	{0x7E, 0x60, 0x60, 0x7C, 0x60, 0x60, 0x60, 0x00},
	// 71  G
	{0x3C, 0x66, 0x60, 0x6E, 0x66, 0x66, 0x3C, 0x00},
	// 72  H
	{0x66, 0x66, 0x66, 0x7E, 0x66, 0x66, 0x66, 0x00},
	// 73  I
	{0x7E, 0x18, 0x18, 0x18, 0x18, 0x18, 0x7E, 0x00},
	// 74  J
	{0x06, 0x06, 0x06, 0x06, 0x66, 0x66, 0x3C, 0x00},
	// 75  K
	{0x66, 0x6C, 0x78, 0x70, 0x78, 0x6C, 0x66, 0x00},
	// 76  L
	{0x60, 0x60, 0x60, 0x60, 0x60, 0x60, 0x7E, 0x00},
	// 77  M
	{0x66, 0x76, 0x7E, 0x7E, 0x66, 0x66, 0x66, 0x00},
	// 78  N
	{0x66, 0x76, 0x7E, 0x7E, 0x6E, 0x66, 0x66, 0x00},
	// 79  O
	{0x3C, 0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00},
	// 80  P
	{0x7C, 0x66, 0x66, 0x7C, 0x60, 0x60, 0x60, 0x00},
	// 81  Q
	{0x3C, 0x66, 0x66, 0x66, 0x6E, 0x3C, 0x06, 0x00},
	// 82  R
	{0x7C, 0x66, 0x66, 0x7C, 0x78, 0x6C, 0x66, 0x00},
	// 83  S
	{0x3C, 0x66, 0x60, 0x3C, 0x06, 0x66, 0x3C, 0x00},
	// 84  T
	{0x7E, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x00},
	// 85  U
	{0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00},
	// 86  V
	{0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x18, 0x00},
	// 87  W
	{0x66, 0x66, 0x66, 0x7E, 0x7E, 0x76, 0x66, 0x00},
	// 88  X
	{0x66, 0x66, 0x3C, 0x18, 0x3C, 0x66, 0x66, 0x00},
	// 89  Y
	{0x66, 0x66, 0x66, 0x3C, 0x18, 0x18, 0x18, 0x00},
	// 90  Z
	{0x7E, 0x06, 0x0C, 0x18, 0x30, 0x60, 0x7E, 0x00},
	// 91  [
	{0x3C, 0x30, 0x30, 0x30, 0x30, 0x30, 0x3C, 0x00},
	// 92  \
	{0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x00},
	// 93  ]
	{0x3C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x3C, 0x00},
	// 94  ^
	{0x18, 0x3C, 0x66, 0x00, 0x00, 0x00, 0x00, 0x00},
	// 95  _
	{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF},
	// 96  `
	{0x30, 0x18, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00},
	// 97  a
	{0x00, 0x00, 0x3C, 0x06, 0x3E, 0x66, 0x3E, 0x00},
	// 98  b
	{0x60, 0x60, 0x7C, 0x66, 0x66, 0x66, 0x7C, 0x00},
	// 99  c
	{0x00, 0x00, 0x3C, 0x66, 0x60, 0x66, 0x3C, 0x00},
	// 100 d
	{0x06, 0x06, 0x3E, 0x66, 0x66, 0x66, 0x3E, 0x00},
	// 101 e
	{0x00, 0x00, 0x3C, 0x66, 0x7E, 0x60, 0x3C, 0x00},
	// 102 f
	{0x1C, 0x30, 0x7C, 0x30, 0x30, 0x30, 0x30, 0x00},
	// 103 g
	{0x00, 0x00, 0x3E, 0x66, 0x66, 0x3E, 0x06, 0x3C},
	// 104 h
	{0x60, 0x60, 0x7C, 0x66, 0x66, 0x66, 0x66, 0x00},
	// 105 i
	{0x18, 0x00, 0x38, 0x18, 0x18, 0x18, 0x3C, 0x00},
	// 106 j
	{0x06, 0x00, 0x0E, 0x06, 0x06, 0x66, 0x66, 0x3C},
	// 107 k
	{0x60, 0x60, 0x66, 0x6C, 0x78, 0x6C, 0x66, 0x00},
	// 108 l
	{0x38, 0x18, 0x18, 0x18, 0x18, 0x18, 0x3C, 0x00},
	// 109 m
	{0x00, 0x00, 0x6C, 0x7E, 0x7E, 0x6A, 0x6A, 0x00},
	// 110 n
	{0x00, 0x00, 0x7C, 0x66, 0x66, 0x66, 0x66, 0x00},
	// 111 o
	{0x00, 0x00, 0x3C, 0x66, 0x66, 0x66, 0x3C, 0x00},
	// 112 p
	{0x00, 0x00, 0x7C, 0x66, 0x66, 0x7C, 0x60, 0x60},
	// 113 q
	{0x00, 0x00, 0x3E, 0x66, 0x66, 0x3E, 0x06, 0x06},
	// 114 r
	{0x00, 0x00, 0x7C, 0x66, 0x60, 0x60, 0x60, 0x00},
	// 115 s
	{0x00, 0x00, 0x3E, 0x60, 0x3C, 0x06, 0x7C, 0x00},
	// 116 t
	{0x30, 0x30, 0x7C, 0x30, 0x30, 0x30, 0x1C, 0x00},
	// 117 u
	{0x00, 0x00, 0x66, 0x66, 0x66, 0x66, 0x3E, 0x00},
	// 118 v
	{0x00, 0x00, 0x66, 0x66, 0x66, 0x3C, 0x18, 0x00},
	// 119 w
	{0x00, 0x00, 0x66, 0x6A, 0x7E, 0x7E, 0x6C, 0x00},
	// 120 x
	{0x00, 0x00, 0x66, 0x3C, 0x18, 0x3C, 0x66, 0x00},
	// 121 y
	{0x00, 0x00, 0x66, 0x66, 0x66, 0x3E, 0x06, 0x3C},
	// 122 z
	{0x00, 0x00, 0x7E, 0x0C, 0x18, 0x30, 0x7E, 0x00},
	// 123 {
	{0x0E, 0x18, 0x18, 0x70, 0x18, 0x18, 0x0E, 0x00},
	// 124 |
	{0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x00},
	// 125 }
	{0x70, 0x18, 0x18, 0x0E, 0x18, 0x18, 0x70, 0x00},
	// 126 ~
	{0x00, 0x00, 0x00, 0x76, 0xDC, 0x00, 0x00, 0x00},
}
